var RetailSegmentHandler = function (db, redis, event) {
    ...
    var modelAndSchemaName = CONTENT_TYPES.RETAILSEGMENT;
    ...
    var BranchesModel = db.model(CONTENT_TYPES.BRANCH, mongoose.Schemas[CONTENT_TYPES.BRANCH]);
    ...
    var branchConst = CONTENT_TYPES.BRANCH;
    var branchSchema = mongoose.Schemas[branchConst];
    var BranchModel = db.model(branchConst, branchSchema);
    ...
    this.getForDD = function (req, res, next) {
        function queryRun() {
            ...
            Model.find(query, '_id name').exec(function (err, response) {
                if (err) {
                    return next(err);
                }
                if (response.length) {
    ...
    this.addConfiguration = function (req, res, next) {
        function queryRun(body) {
            ...
            Model.findById(body.retailId, function (err, model) {
                var configurations;
                var error;
                ...
                Model.findByIdAndUpdate(body.retailId, {$push: {configurations: saveObj}}, {new: true}, function (err, model) {
                    if (err) {
                        return next(err);
                    }
                    if (model && model.name) {
