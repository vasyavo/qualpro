var Documents = function (db, redis, event) {
        ...
        function queryRun(updateObject) {
            ...
            async.waterfall([
                function (cb) {
                    ...
                    Model.findByIdAndUpdate(documentId, updateObject, function (err, result) {
                        if (err) {
                            return cb(err);
                        }
                        // event.emit('activityChange', {
    ...
    this.archive = function (req, res, next) {
        function queryRun() {
            ...
            var ContractSecondaryModel = db.model(CONTENT_TYPES.CONTRACTSSECONDARY, mongoose.Schemas[CONTENT_TYPES.CONTRACTSSECONDARY])
            var ContractYearlyModel = db.model(CONTENT_TYPES.CONTRACTSYEARLY, mongoose.Schemas[CONTENT_TYPES.CONTRACTSYEARLY])
            var options;
