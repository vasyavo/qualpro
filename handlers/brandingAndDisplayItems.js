var Promotions = function (db, redis, event) {
    ...
    var modelAndSchemaName = CONTENT_TYPES.BRANDINGANDDISPLAYITEMS;
    var schema = mongoose.Schemas[modelAndSchemaName];
    var Model = db.model(modelAndSchemaName, schema);
    var brandingAndDisplaySchema = mongoose.Schemas[CONTENT_TYPES.BRANDINGANDDISPLAY];
    var BrandingAndDisplayModel = db.model(CONTENT_TYPES.BRANDINGANDDISPLAY, brandingAndDisplaySchema);
    ...
    this.getAll = function (req, res, next) {
        ...
        access.getReadAccess(req, 40, function (err, allowed, personnel) {
            if (err) {
                return next(err);
            }
    ...
    this.create = function (req, res, next) {
        function queryRun(body) {
            ...
            async.waterfall([
                ...
                function (model, callback) {
                    ...
                    commentCreator(saveObj, Model, function (err, comment) {
                        if (err) {
                            return callback(err);
                        }
                ...
                function (model, comment, callback) {
                    Model.findByIdAndUpdate(model._id, {$addToSet: {comments: comment._id}}, {new: true}, callback);
                }
            ], function (err, result) {
                if (err) {
                    return next(err);
        ...
        access.getWriteAccess(req, 40, function (err, allowed) {
            var body;
