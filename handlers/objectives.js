var Objectives = function (db, redis, event) {
    ...
    var ObjectiveHistory = db.model(CONTENT_TYPES.OBJECTIVEHISTORY, mongoose.Schemas[CONTENT_TYPES.OBJECTIVEHISTORY]);
    var FilterMapper = require('../helpers/filterMapper');
    var FileHandler = require('../handlers/file');
    var fileHandler = new FileHandler(db);
    var access = require('../helpers/access')(db);
    var ObjectId = mongoose.Types.ObjectId;
    var fileModelAndSchemaName = CONTENT_TYPES.FILES;
    var fileSchema = mongoose.Schemas[fileModelAndSchemaName];
    var File = db.model(fileModelAndSchemaName, fileSchema);
                ...
                function updateParentObjective(id, seriesCb) {
                    async.waterfall([
                        function (waterFallCB) {
                            Model.findById(id, function (err, parentModel) {
                                if (err) {
                        ...
                        function (parentModel, waterFallCB) {
                            ...
                            Model.find(query, function (err, childModels) {
                                var complete;
    ...
    this.removeFileFromObjective = function (req, res, next) {
        ...
        async.waterfall([
            ...
            function (cb) {
                File.findById(fileId, function (err, fileModel) {
                    if (err) {
                        return cb(err, null);
                    }
            ...
            function (removeFile, cb) {
                Model.update(objectiveId, {$pull: {attachments: ObjectId(fileId)}}, function (err) {
                    if (err) {
                        return cb(err);
                    }
        ...
        function queryRun(updateObject, body) {
            ...
            function updateCover(objective) {
                PersonnelModel.findOne({
                    ...
                    objectiveHistory = new ObjectiveHistory(history);
            ...
            function updateObjective(filesIds, waterFallCb) {
                ...
                Model.findOneAndUpdate({_id: objectiveId}, fullUpdate, {new: true}, function (err, objectiveModel) {
                    if (err) {
                        return waterFallCb(err);
                    }
            ...
            function updateChildObjective(options) {
                ...
                Model.update({_id: {$in: nextObjectiveIds}}, {$set: {'createdBy.user': ObjectId(assignedTo[0])}}, function (err) {
                    if (err) {
                        return consoleLogENV(err);
                    }
                });
                ...
                Model.find(query, function (err, models) {
                    ...
                    if (models) {
                        PersonnelModel.findById(updateObject.assignedTo[0], function (err, personnel) {
                            ...
                            Model.findByIdAndUpdate(objectiveModelId, {
                                $set: {
    ...
    this.deleteByIds = function (req, res, next) {
        ...
        function getParallelFunction(id) {
            return function (callback) {
                Model.findByIdAndRemove(id, function (error, result) {
                    if (error) {
                        return callback(error);
                    }
    ...
    this.getByIdHistory = function (req, res, next) {
        function queryRun() {
            ...
            aggregation = ObjectiveHistory.aggregate(pipeline);
