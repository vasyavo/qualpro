...
var Domain = function (db, redis, event) {
    ...
    var archiver = new Archiver(Model, BranchModel, event);
    ...
    this.getForDD = function (req, res, next) {
        function queryRun() {
            ...
            Model.find(query, '_id name parent').exec(function (err, result) {
                if (err) {
                    return next(err);
                }
                if (result.length) {
    ...
    this.update = function (req, res, next) {
        ...
        function queryRun(body) {
            ...
            query = Model.findByIdAndUpdate(id, body, {new: true});
