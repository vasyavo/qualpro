...
module.exports = (req, res, next) => {
    ...
    async.waterfall([
        ...
        response.formTypes = formTypes.filter((item) => {
            return response.formTypes.indexOf(item._id) > -1;
        });

        response.priorities = priorities.filter((item) => {
            return response.priorities.indexOf(item._id) > -1;
        });
